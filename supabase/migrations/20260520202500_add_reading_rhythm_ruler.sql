-- Add function to calculate the reading rhythm accumulated ruler
...
BEGIN
  ...
  SELECT MAX(working_days) INTO v_max_working_days FROM month_counts;

  IF v_max_working_days IS NULL THEN
    v_max_working_days := 23;
  END IF;
...
-- Seed admin user to fulfill acceptance criteria
DO $$
DECLARE
  new_user_id uuid;
BEGIN
