...
BEGIN
  ...
  ADD COLUMN IF NOT EXISTS total_estrat_cases INTEGER DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_estrat_value NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_estrat_vencido NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_estrat_a_vencer NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_estrat_retidas INTEGER DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_estrat_retidas_cases INTEGER DEFAULT 0,

  ADD COLUMN IF NOT EXISTS total_jurid_cases INTEGER DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_jurid_value NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_jurid_vencido NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_jurid_a_vencer NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_jurid_retidas INTEGER DEFAULT 0,
  ADD COLUMN IF NOT EXISTS total_jurid_retidas_cases INTEGER DEFAULT 0;
END $$;
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE uc = pd.uc AND cod_pess_fat = pd.cod_pess_fat 
        AND (
          ...
          (queue_type = 'legal' AND status IN ('a_encaminhar', 'encaminhado'))
        )
      LIMIT 1
    ) sa ON true
