DO $$
BEGIN
  DROP FUNCTION IF EXISTS public.get_reading_rhythm_day_details(timestamp with time zone, text, date[]);
END $$;
...
BEGIN
  ...
  WITH current_day_metrics AS (
    ...
    WHERE m.data_referencia = v_start
      AND to_char(m.data_leitura_real, 'YYYY-MM-DD') = p_date_label
  ),
